...
impl<'a> WxEntry<'a, LayerHash<'a>> for HashMapWx {
    ...
    fn layer(&self, layer: Layer) -> Option<LayerHash<'_>> {
        Some(LayerHash { layer, data: self })
    }
...
impl<'a> WxEntryLayer for LayerHash<'a> {
    ...
    fn temperature(&self) -> Option<Temperature> {
        self.get(Param::Temperature)
    }
    fn pressure(&self) -> Option<Pressure> {
        self.get(Param::Pressure)
    }
    fn visibility(&self) -> Option<Distance> {
        self.get(Param::Visibility)
    }
    ...
    fn dewpoint(&self) -> Option<Temperature> {
        ...
        self.get(Param::Dewpoint).or({
            Some(dewpoint_from_rh(
                self.get(Param::Temperature)?,
                self.get(Param::RelativeHumidity)?,
            ))
        })
    }
    fn relative_humidity(&self) -> Option<Fraction> {
        self.get(Param::RelativeHumidity).or({
            Some(rh_from_dewpoint(
                self.get(Param::Temperature)?,
                self.get(Param::Dewpoint)?,
            ))
        })
    }

    fn wind_speed(&self) -> Option<Speed> {
        self.get(Param::WindSpeed)
            .or(Some(self.get::<Wind>(Param::Wind)?.speed))
    }
    ...
    fn wind(&self) -> Option<Wind> {
        self.get(Param::Wind).or(Some(Wind {
            direction: self.get(Param::WindDirection)?,
            speed: self.get(Param::WindSpeed)?,
        }))
    }
}
